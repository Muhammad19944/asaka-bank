<template>
	<div class="slides-view">
		...
		<app-card :title="$t('Dashboard.sidebar.slide_section')" class="mb-13">
			<template #add-view>
				<v-btn :to="{ name: 'OtherSectionSlidesCreate' }" depressed color="primary">
			...
			<template v-if="slidesList.length">
				<v-simple-table class="admin-table">
					<tbody>
					<tr class="transition-fast-out-slow-in" v-for="item in slidesList" :key="item.id">
						<td class="admin-table-td">
							<h5 class="font-600">{{ item.name_uz }}</h5>
							<span class="text-small font-600 text-secondary-dark">Slide nomi</span>
						</td>
...
<!--							<v-btn-->
<!--								:to="{ name: 'DashboardfilialRead', params: { id: item.id } }"-->
<!--								depressed-->
<!--								icon-->
<!--								color="info"-->
<!--							>-->
...
<!--							<v-btn-->
<!--								:to="{ name: 'DashboardfilialUpdate', params: { id: item.id } }"-->
<!--								depressed-->
<!--								icon-->
<!--								color="warning"-->
<!--								class="mx-2"-->
...
export default {
	...
	methods: {
		 ...
		 * */
		getAll() {
			this.$http.get(`/${this.$store.state.auth.COMPANY_ID}/slide/image/`).then(({ data }) => {
				this.slidesList = data.results;
		...
		deleteRow(id) {
			...
			this.$http
				.delete(`/${this.$store.state.auth.COMPANY_ID}/slide/image/${id}/`)
				.then(() => {
