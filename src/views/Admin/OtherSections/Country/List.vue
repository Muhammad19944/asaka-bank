<template>
	<div class="country-view">
		...
		<app-card :title="$t('Dashboard.sidebar.list_of_country')" class="mb-13">
			<template #add-view>
				<v-btn :to="{ name: 'OtherSectionCountryCreate' }" depressed color="primary">
			...
			<template v-if="countryList.length">
				<v-simple-table class="admin-table">
					<tbody>
					<tr class="transition-fast-out-slow-in" v-for="item in countryList" :key="item.id">
						<td class="admin-table-td">
							<h5 class="font-600">
								<I18nVue :uz='item.name_uz' :ru='item.name_ru' :en='item.name_en' />
							</h5>
							<span class="text-small font-600 text-secondary-dark">{{ $t("Dashboard.country.name") }}</span>
						</td>
						...
						<td class="admin-table-td" width="160">
							<v-btn
								:to="{ name: 'OtherSectionCountryRead', params: { id: item.id } }"
								depressed
								icon
								color="info"
							>
							...
							<v-btn
								:to="{ name: 'OtherSectionCountryUpdate', params: { id: item.id } }"
								depressed
								icon
								color="warning"
								class="mx-2"
...
export default {
	...
	methods: {
		...
		deleteRow(id) {
			...
			this.$http
				.delete(`/${this.$store.state.auth.COMPANY_ID}/country/${id}/`)
				.then(() => {
