<template>
  <div class="district-view">
    <app-layout-scope-renderless
        api-url="bank/finance_indication_title"
        category="5"
    >
      <template slot-scope="{ filters, list, count, loading, getAll, deleteRow, paginate }">
        <div class="d-flex flex-column h-100">
          ...
          <app-card :title="$t('Dashboard.sidebar.annual_documents')" class="mb-13">
            <template #add-view>
              <v-btn :to="{ name: 'OtherSectionAnnualDocumentsCreate' }" depressed color="primary">
              ...
              <v-simple-table class="admin-table">
                ...
                <tr class="transition-fast-out-slow-in" v-for="item in list" :key="item.id">
                  ...
                  <td class="admin-table-td" width="160">
                    <v-btn
                        :to="{ name: 'OtherSectionAnnualDocumentsRead', params: { id: item.id } }"
                        depressed
                        icon
                        color="info"
                    >
                    ...
                    <v-btn
                        :to="{ name: 'OtherSectionAnnualDocumentsUpdate', params: { id: item.id } }"
                        depressed
                        icon
                        color="warning"
                        class="mx-2"
