<template>
	<div class='form-wrap'>
		<template v-if="$store.state.CREATED_ANNUAL_DOCUMENT_ID === null">
			<v-col md="12">
								...
								<v-tooltip top color="primary">
									<template v-slot:activator="{ on, attrs }">
										<v-btn
											...
											:disabled="$store.state.CREATED_ANNUAL_DOCUMENT_ID === null || TYPE === 'Read'"
											color="primary"
											large
											dark
											v-bind="attrs"
...
<script>
	...
	export default {
		...
		methods:{
			getAll(){
				this.$http.get(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/${this.$store.state.CREATED_ANNUAL_DOCUMENT_ID}/many_documents/`)
				.then(({ data }) => {
			...
			* */
			send(){
				...
				this.$http.post(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/${this.$store.state.CREATED_ANNUAL_DOCUMENT_ID}/many_documents/`, this.documentsModel)
				.then(({ data }) => {


					// data.document.push(this.documentsModel);
			...
			* */
			update(){
				...
				if (this.$route.params.id){
					this.$store.state.CREATED_ANNUAL_DOCUMENT_ID = this.$route.params.id
				}

				this.$http.put(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/${this.$store.state.CREATED_ANNUAL_DOCUMENT_ID}/many_documents/${this.documentsModel.id}/`, this.documentsModel)
				.then((res) => {
			...
			* */
			deleteProp(id) {
				...
				this.$http
					.delete(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/${this.$store.state.CREATED_ANNUAL_DOCUMENT_ID}/many_documents/${id}/`)
					.then((res) => {
			...
			clearFields(){
				this.documentsModel.title_uz = '';
				this.documentsModel.title_ru = '';
				this.documentsModel.title_en = '';
				this.documentsModel.file = null
			}
		}
	}
</script>
