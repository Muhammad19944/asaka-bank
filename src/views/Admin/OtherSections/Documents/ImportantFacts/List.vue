<template>
	<div class="district-view">
		...
		<app-card :title="$t('AboutBank.important_facts')" class="mb-13">
			<template #add-view>
				<v-btn :to="{ name: 'OtherSectionImportantFactsCreate' }" depressed color="primary">
				...
				<v-simple-table class="admin-table">
					...
					<tr class="transition-fast-out-slow-in" v-for="item in documentsList" :key="item.id">
						...
						<td class="admin-table-td" width="300">
							...
							<span class="text-small font-600 text-secondary-dark d-block">{{ $t("Dashboard.branch.type") }}</span>
						</td>
						<td class="admin-table-td" width="300">
							<h5 class="font-600">
						...
						<td class="admin-table-td" width="160">
							<v-btn
								:to="{ name: 'OtherSectionImportantFactsRead', params: { id: item.id } }"
								depressed
								icon
								color="info"
							>
							...
							<v-btn
								:to="{ name: 'OtherSectionImportantFactsUpdate', params: { id: item.id } }"
								depressed
								icon
								color="warning"
								class="mx-2"
...
export default {
	...
	methods: {
		 ...
		 * */
		getAll() {
			this.$http.get(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/`, {
				params: {
		...
		deleteRow(id) {
			...
			this.$http
				.delete(`/${this.$store.state.auth.COMPANY_ID}/bank/finance_indication_title/${id}/`)
				.then(() => {
