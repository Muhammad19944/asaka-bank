		    ...
		    <template slot-scope="{ filters, list, count, loading, getAll, deleteRow, paginate }">
			    <div class="d-flex flex-column h-100">
				    ...
				    <app-card :title="$t('Dashboard.sidebar.list_of_loans')" class="mb-13">
					    <template #add-view>
						    <v-btn :to="{ name: 'AdminCorporateCreditsCreate' }" depressed color="primary">
						    ...
						    <v-simple-table class="admin-table">
							    ...
							    <tr class="transition-fast-out-slow-in" v-for="item in list" :key="item.id">
								    ...
								    <td class="admin-table-td">
									    ...
									    <span class="text-small font-600 text-secondary-dark">{{ $t('Dashboard.credits.initial_fee') }}</span>
								    </td>
								    <td class="admin-table-td">
								    ...
								    <td class="admin-table-td" width="160">
									    <v-btn
										    :to="{ name: 'AdminCorporateCreditsRead', params: { id: item.id } }"
										    depressed
										    icon
										    color="info"
									    >
									    ...
									    <v-btn
										    :to="{ name: 'AdminCorporateCreditsUpdate', params: { id: item.id } }"
										    depressed
										    icon
										    color="warning"
										    class="mx-2"
