				...
				<v-simple-table class="admin-table">
					...
					<tr class="transition-fast-out-slow-in" v-for="item in appealList" :key="item.id">
						<td class="admin-table-td">
							<h5 class="font-600">{{ item.appeal_type && item.appeal_type.name_uz }}</h5>
							<span class="text-small font-600 text-secondary-dark">{{ $t("Services.OpenCard.typeApplication") }}</span>
						</td>
						<td class="admin-table-td">
							<I18nVue
								:uz='item.filial && item.filial.name_uz'
								:ru='item.filial && item.filial.name_ru'
								:en='item.filial && item.filial.name_en'
						...
						<td class="admin-table-td" width="75">
							<v-btn :to="{ name: 'FeedbackAppealRead', params: { id: item.id } }" depressed icon
							       color="info">
...
export default {
	...
	methods: {
		 ...
		 * */
		getAll() {
			this.$http.get(`/${this.$store.state.auth.COMPANY_ID}/appeal/action/`).then(({ data }) => {
