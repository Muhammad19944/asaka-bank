		...
		<app-card>
			<div class="form-wrap pa-5 px-6">
				<div class="d-flex align-center mb-8">
					<h4 class="text--primary text-uppercase font-700 mr-4">{{ $t("Dashboard.read") }}</h4>
							...
							<v-expansion-panel-content>
								<div class="form-wrap-body">
									<v-row>
										...
										<v-col md="4">
											<div class="app-form-group app-form-group__small no-shadow mb-2">
												<h5 class="font-600 dark-text mb-1">{{ $t("Services.OpenCard.stir") }}</h5>

												<v-text-field
													v-model="applicationModel.zip_code"
													outlined
													readonly
													hide-details
												>
										...
										<v-col md="4">
											<div class="app-form-group app-form-group__small no-shadow mb-2">
												<h5 class="font-600 dark-text mb-1">{{ $t("Dashboard.branch.name") }}</h5>
										...
										<template v-if="applicationModel.file">
											<v-col md="4">
												<file-upload
													ref="propFile"
													:value="passportFile"
...
export default {
	...
	methods: {
		...
		* */
		getOne() {
			this.$http.get(`/8/appeal/action/${this.$route.params.id}/`)
			.then(({ data }) => {
				this.applicationModel = data;
				this.applicationRequestModel.text = data.application_outcome.text
				this.passportFile = this.applicationModel.file
			})
		},
        ...
        * */
		changeStatus(val) {
			this.$http
				.put(`/1/appeal/action/${this.$route.params.id}/`, {
					status: val
				})
				.then(() => {
        ...
        * */
		moveToFilial() {
			this.$http
				.put(
					`/8/appeal/action/${this.$route.params.id}/transfer_to_filial/${this.selectFilial}/`
				)
				.then(() => {
					this.$toast.success(`Filialga yuborildi`);

					setTimeout(() => {
						this.$router.push({ name: "FeedbackAppealList" });
					}, 1000);
				})
				.catch(() => {
