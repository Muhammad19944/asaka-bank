<template>
	<div class="announcements-view">
		<div class="d-flex align-center mb-5">
			<h5 class="app-card-title font-600 text-secondary-dark">E'lonlar</h5>

			<v-spacer />

			<v-btn :to="{ name: 'DashboardAnnouncementsCreate' }" depressed color="primary">
		...
		<v-row>
			<v-col md="9">
				<v-row>
					<template v-for="(item, index) in announcementsList">
						<v-col md="4" :key="index">
							...
							>
								<div class="app-card-header p-relative">
									...
									>
										<router-link
											:to="{ name: 'DashboardAnnouncementsRead', params: { id: item.id } }"
											type="button"
											class="
                                                rounded-circle
                                                d-flex
										...
										<router-link
											:to="{ name: 'DashboardAnnouncementsUpdate', params: { id: item.id } }"
											type="button"
											class="
                                                rounded-circle
                                                d-flex
				...
				this.$http
					...
					.then(({ data }) => {
						...
						data.results.forEach(item => {
							this.announcementsList.push(item)
						})
					})
			},
			/*
