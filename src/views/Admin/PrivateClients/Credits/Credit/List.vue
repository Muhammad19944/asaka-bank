			...
			<template slot-scope="{ filters, list, count, loading, getAll, deleteRow, paginate }">
				<div class="d-flex flex-column h-100">
					...
					<app-card :title="$t('Dashboard.sidebar.list_of_loans')" class="mb-13">
						<template #add-view>
							<v-btn :to="{ name: 'PrivateClientsCreditsCreate' }" depressed color="primary">
								...
								<tbody>
									<tr class="transition-fast-out-slow-in" v-for="item in list" :key="item.id">
										...
										<td class="admin-table-td" width="350">
											...
											<span class="text-small font-600 text-secondary-dark">{{ $t("Dashboard.credits.name")
												}}</span>
										</td>
										...
										<td class="admin-table-td" width="250">
											...
											<span class="text-small font-600 text-secondary-dark">{{ $t("Dashboard.credits.period")
												}}</span>
										</td>
										<td class="admin-table-td">
											<h5 class="font-600">{{ item.credit_initial_fee }}</h5>
											<span class="text-small font-600 text-secondary-dark">{{ $t("Dashboard.credits.initial_fee")
												}}</span>
										</td>
										<td class="admin-table-td">
										...
										<td class="admin-table-td" width="160">
											<v-btn
												:to="{ name: 'PrivateClientsCreditsRead', params: { id: item.id } }"
												depressed
												icon
												color="info"
											>
											...
											<v-btn
												:to="{ name: 'PrivateClientsCreditsUpdate', params: { id: item.id } }"
												depressed
												icon
												color="warning"
												class="mx-2"
