<template>
	<div class="internal-audit-view">
		<v-container>
			<div class="app-view-content">
				<v-divider style="border-color: rgba(var(--border-color), 1)" class="mt-5 mb-8"></v-divider>
			</div>
			<template v-for="item in internalAuditList">
				<v-row>
					<v-col v-if="$i18n.locale === 'uz' && item.file.url" md="6">
						<a
							:key="item.id"
							:href="$http.defaults.baseURL + item.file.url"
							target="_blank"
							rel="noopener noreferrer"
							class="credit-widget pa-7 rounded-lg overflow-hidden d-flex align-center mb-8 text-decoration-none"
						>
					...
					<v-col v-if="$i18n.locale === 'ru' && item.file_ru.url" md="6">
						<a
							:key="item.id"
					...
					<v-col v-if="$i18n.locale === 'en' && item.file_en.url" md="6">
						<a
							:key="item.id"
...
<script>
	...
	export default {
		...
		mounted() {
			this.getInternalAuditDocuments();
		},
		methods: {
			getInternalAuditDocuments(){
				...
				.then(({ data }) => {
					this.internalAuditList = data.results
				})
			}
		}
	}
