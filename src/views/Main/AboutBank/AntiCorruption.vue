	...
	<div class="anti-corruption-view">
		<v-container>
			<div class="app-content-view">
				<h2 class="font-600 dark-text">{{ $t("toDownload") }}</h2>

				<v-divider style="border-color: rgba(var(--border-color), 1)" class="mt-5 mb-8"></v-divider>

				<v-row>
					<v-col md="6" v-for="item in documentsList" :key="item.id">
						<template>
							<a
								target="_blank"
								rel="noopener noreferrer"
								:href="$i18n.locale === 'uz' ?
									$http.defaults.baseURL + item.file.url :
									$i18n.locale === 'ru' ?
									$http.defaults.baseURL + item.file_ru.url :
									$http.defaults.baseURL + item.file_en.url
								"
								class="
                                    app-slide-card
                                    pa-6
