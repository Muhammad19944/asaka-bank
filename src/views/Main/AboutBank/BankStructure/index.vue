<template>
	<div class="bank-structure-view">
		<div class="view-toolbar px-4 mt-16">
          ...
          >
            <div
                @click="getSelectedList('bank_council')"
                class="tab-header-items text-center mr-7"
                :class="{'active' : bankStructureRoute}"
                data-item="header"
            >
          ...
          >
            <div
                ...
                class="tab-header-items text-center mr-7"
                :class="{'active' : structuralUnitsRoute}"
                data-item="header"
            >
              <a class=" navigation-link navigation-link__hover p-relative text--h4 py-6 font-600 d-inline-block">
          ...
          >
            <div
                class="tab-header-items text-center mr-7"
                :class="{'active' : bankStructureDiagramRoute}"
                data-item="header"
            >
              <a class=" navigation-link navigation-link__hover p-relative text--h4 py-6 font-600 d-inline-block">
          ...
          >
            <div
                @click="getSelectedList('audit_commission')"
                class="tab-header-items text-center mr-7"
                :class="{'active' : bankAuditCommitteeRoute}"
                data-item="header"
            >
              <a class="navigation-link navigation-link__hover p-relative text--h4 py-6 font-600 d-inline-block">
			...
			<v-container>
				<div class="app-tab-content p-relative">
					...
					<div
						...
						data-item="content"
					>
						<img draggable="false" :src="$i18n.locale === 'uz' ? '/images/struktura-uz.jpg' : '/images/struktura-ru.png'" alt="Structure image" />
					</div>
...
export default {
	...
	data() {
		return {
			...
			structureList: [],
			departmentList: [],
			bankStructureRoute: false,
			bankBoardRoute: false,
			structuralUnitsRoute: false,
			bankStructureDiagramRoute: false,
			bankAuditCommitteeRoute: false
		}
	},
	watch: {
		'$route'() {
	...
	methods: {
		...
		* */
		getList(param) {
          ...
          this.structureList.sort((a, b) => (a.rank > b.rank ? 1 : -1))
				});
		},
		...
		* */
		getDepartmentList() {
			this.$http.get("/1/department/?ordering=id", {
				params: {
					page_size: 500
				}
			})
        ...
        // SORT BY MEMBER RANK AND DISPLAY IN THE LIST
          this.departmentList.forEach(item => {
            item.members.sort((a, b) => (a.rank > b.rank ? 1 : -1))
          })
        });
		},
		...
		* */
		setActiveRoute() {
			switch (this.$route.name) {
				case "BankStructure" :
					this.bankStructureRoute = true
					this.bankBoardRoute = this.structuralUnitsRoute = this.bankStructureDiagramRoute = this.bankAuditCommitteeRoute = false
					this.getSelectedList("bank_council")
					break;
				case "BankBoard" :
					this.bankBoardRoute = true
					this.bankStructureRoute = this.structuralUnitsRoute = this.bankStructureDiagramRoute = this.bankAuditCommitteeRoute = false
					this.getSelectedList()
					break;
				case "StructuralUnits" :
					this.structuralUnitsRoute = true
					this.bankBoardRoute = this.bankStructureRoute = this.bankStructureDiagramRoute = this.bankAuditCommitteeRoute = false
					this.getDepartmentList()
					break;
				case "BankStructureDiagram" :
					this.bankStructureDiagramRoute = true
					this.bankBoardRoute = this.bankStructureRoute = this.structuralUnitsRoute = this.bankAuditCommitteeRoute = false
					break;
				case "BankAuditCommittee" :
					this.bankAuditCommitteeRoute = true
					this.bankBoardRoute = this.bankStructureRoute = this.bankStructureDiagramRoute = this.structuralUnitsRoute = false
					this.getSelectedList()
					break;
				default :
					break;
