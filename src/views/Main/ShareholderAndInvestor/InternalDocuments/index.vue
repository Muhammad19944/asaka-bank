    ...
    <v-container>
      <div class="app-view-content">
        <h2 class="font-600 dark-text">
          <I18nVue t="SHAREHOLDERS_AND_INVESTORS.InternalDocuments"/>
        </h2>
        <v-divider style="border-color: rgba(var(--border-color), 1)" class="mt-5 mb-8"></v-divider>
        <v-row>
          <v-col md="6" v-for="(item, idx) of inner_documents">
            <a
              v-if="inner_documents"
              :key="idx" :id="idx"
...
export default {
  name: "index",
  components: { I18nVue },
  data: () => ({ inner_documents: null }),
  mounted() {this.$http.get("/1/bank/documents/?type=inner").then(({ data }) => this.inner_documents = data.results)}
};
</script>
