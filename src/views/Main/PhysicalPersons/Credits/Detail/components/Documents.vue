		...
		<v-row>
			<v-col v-if='documents' md='6'>
				<h2 class='font-600 dark-text mb-8'>
					<I18nVue t='Credit_documents_title'/>
				</h2>
				<div
					v-for='item of notDownloadList'
				...
				>
					...
					<h3 class='dark-text font-600'>
						...
						<I18nVue
							:uz='item.title_uz && item.title_uz'
							:ru='item.title_ru && item.title_ru'
							:en='item.title_ru && item.title_ru'
						/>
						<br/>
						<I18nVue
							classes='font-300 font-italic'
							:uz='item.doc_name_uz && item.doc_name_uz'
							:ru='item.doc_name_ru && item.doc_name_ru'
							:en='item.doc_name_en && item.doc_name_en'
						/>
					</h3>
				</div>
			</v-col>
			...
			<v-col md='6'>
				...
				<template v-for='item of forDownloadList'>
					<a
						v-if='item.for_download !== null'
						target='_blank'
						download
						class='app-slide-card pa-7 mb-8 rounded-lg overflow-hidden p-relative d-block text-decoration-none border-all'
						style='min-height: auto'
						:href='$http.defaults.baseURL + item.for_download.url'
					>
						<div class='d-flex align-center'>
							<div class='mr-5 p-relative'>
								<v-avatar color='#fff' size='56'>
							...
							<div class='flex-1'>
								<h3 v-if="$i18n.locale === 'uz'" class='dark-text font-600'>{{ item.title_uz }}</h3>
								<h3 v-if="$i18n.locale === 'ru'" class='dark-text font-600'>{{ item.title_ru }}</h3>
								<h3 v-if="$i18n.locale === 'en'" class='dark-text font-600'>{{ item.title_en }}</h3>
							</div>
...
export default {
	...
	computed: {
		notDownloadList() {
			return this.depositDetailModel.filter(document => document.for_download === null)
		},
		forDownloadList() {
			return this.depositDetailModel.filter(document => !!document.for_download)
		}
	},
